...
int sys_close(int fd);		// 关闭指定文件  fs/open.c
...
// priv: 发送信号的标志。即不需要考虑进程用户属性或级别而能发送信号的权利。
...
volatile void do_exit(long code)
{
	...
	// 释放当前进程代码段和数据段所占的内存页
	// free_page_tables 在 mm/memory.c
	...
	// 对当前进程的工作目录、根目录、程序的 i 节点进行同步，并置空
	iput(current->pwd);
	current->pwd = NULL;
	iput(current->root);
	current->root = NULL;
		...
		switch (p->state) {
			case TASK_STOPPED:
				if (!(options & WUNTRACED) || 
				    !p->exit_code)
					continue;
				// 则把状态信息 0x7f 放入*stat_addr
				// 0x7f 表示的返回状态使 WIFSTOPPED()宏为真, include/sys/wait.h
			...
			case TASK_ZOMBIE:
				...
				return flag;
			default:
				// 如果子进程 p 的状态既不是停止也不是僵死，则 flag 置 1
